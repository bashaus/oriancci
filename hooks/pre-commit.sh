...
PSR1_STDOUT=`$ROOT/hooks/pre-commit/psr-1.sh`
PSR1_RESULT=$?
echo "$PSR1_STDOUT"

# Check PSR-2 standard
PSR2_STDOUT=`$ROOT/hooks/pre-commit/psr-2.sh`
PSR2_RESULT=$?
echo "$PSR2_STDOUT"

# Check the JSON file
JSON_STDOUT=`/usr/bin/env php $ROOT/hooks/pre-commit/composer-json.php`
JSON_RESULT=$?
echo "$JSON_STDOUT"
